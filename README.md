...
| 这篇论文是什么 | 3 | 决定论文本身 |
| 数字取决于什么 | 3 | 改变发表的数字 |
| 什么进论文、什么进手册 | 3 | 位置问题，不是要不要写 |
| 发布清单 | 3 | 是任务，不是问题 |

**第一组才是重点，而且一个字的代码都改不出来。**

整棵树关于 CBS 是干什么的，只说过一次，在一个文件头里（`solo.cpp:4`）：*"ColliderBit Solo: an event-based LHC recast tool using the GAMBIT ColliderBit module"*。这是描述，不是论断，而且**没有第二处**。同一个分支能写出至少四篇不同的论文，它们不共享结果、不共享对比对象、也不共享读者：(a) 一个独立的 recast 工具；(b) 一个让 ColliderBit 脱离 GAMBIT scan 也能用的接口；(c) 一批新分析加 VR jet；(d) 一份可复现的结果契约。代码的重心并不在名字指的地方——runner 只有 **10 个文件**，分析那边是 **162 个**。

另外两条同组：**读者是谁**（GAMBIT 老用户要的是 delta，没装过 GAMBIT 的人第一页就得面对 69 个 backend，做下游 pipeline 的只要输出契约），以及 **拿什么比**（树里没有任何一处把 CBS 和别的框架作过对比）。比吞吐量或探测器精度，CBS 输给专门干这个的工具；比**拿到结果之后能做什么**——关联 likelihood、histogram 变出的 per-bin signal region、机器可读的契约——这块地基本没人占。**选轴就是选论点。**

改变数字的三条：

- **`check_histogram` 默认 `false`**（`solo.cpp:179`）。打开之后两个 SR histogram 分析多提交 **23 个 signal region**。这不是 verbosity flag，它改变 likelihood 项的数量。
- **VR jet 从不做 smearing**。它们因为"是 variable-R"而被自动排除在 JER 之外（`BuckFast.cpp:48,56`，名单在 `getBuckFast.cpp` 三处填）。要么是有意的（实验端已做标定，再抹一次是错的）——一句话就能了结；要么是个缺口——那就是两个 VR 分析上一项没量化的系统误差。
- **十二个新分析里哪些验证过？** 5 个完全没有 `Cutflow` 仪表，3 个在自己的注释里写着没做完（`ATLAS_SUSY_2018_07:41` 写着 *"Not validated, still in progress"*）。

**发布清单单独放在最下面**，明确标注"是任务不是问题"：Contur 版本自相矛盾、把默认卡片打包进发布、给安装路线起个名字。这些都有显然的修法，有的发布本身就顺手做了——列在这里只是因为上面某条论断目前还压在它们身上。特别是默认卡片那条，真正的任务不是打包，而是**缺文件时要大声报错**，以及**把解析后的设置写进输出 JSON**，让每次运行自带它跑在什么配置下——顺带把 `check_histogram` 那条也一并解决了。
